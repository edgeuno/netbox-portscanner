...
NAME = os.getenv("IMAGE_NAME", "nm-netbox-plugin-portscanner")
PWD = os.getcwd()

COMPOSE_FILE = "development/docker-compose.yml"
BUILD_NAME = "netbox_portscanner"
...
def destroy(context, netbox_ver=NETBOX_VER, python_ver=PYTHON_VER):
    ...
    context.run(
        f"docker volume rm -f {BUILD_NAME}_pgdata_netbox_portscanner",
        env={"NETBOX_VER": netbox_ver, "PYTHON_VER": python_ver},
    )
...
def unittest(context, netbox_ver=NETBOX_VER, python_ver=PYTHON_VER):
    ...
    context.run(
        f'{docker} sh -c "python manage.py test netbox_portscanner"',
        env={"NETBOX_VER": netbox_ver, "PYTHON_VER": python_ver},
        pty=True,
    )
